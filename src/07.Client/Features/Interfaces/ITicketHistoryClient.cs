...
namespace SupportTicketSystem.Client.Features.Interfaces
{
    public interface ITicketHistoryClient
    {
        ...
        Task<byte[]> ExportExcelAsync(Guid? ticketId, string? action, Guid? changedBy, string? search, DateTime? startDate, DateTime? endDate);

        Task<byte[]> ExportPdfAsync(Guid? ticketId, string? action, Guid? changedBy, string? search, DateTime? startDate, DateTime? endDate);
    }
}